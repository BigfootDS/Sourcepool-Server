...
router.get("/all", async (request, response) => {
	let result = await InGameDate.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await InGameDate.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await InGameDate.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await InGameDate.find({});

	switch (request.params.key) {
		case "name":
			results = await InGameDate.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await InGameDate.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
/*
{
    ...
    "description": [
        {
            "language":"en",
            "name":"Testo InGameDate",
            "content":"Test of functionality."
        },
                {
            "language":"fr",
            "name":"InGameDate de Testo",
...
router.post("/one", async (request, response) => {
	// let newAbility = await InGameDate.create({
	...
	let newDoc = await InGameDate.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await InGameDate.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await InGameDate.findOneAndDelete({_id: request.body.targetDocId});
