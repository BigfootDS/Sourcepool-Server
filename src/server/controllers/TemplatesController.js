...
router.get("/all", async (request, response) => {
	let result = await Template.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await Template.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await Template.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await Template.find({});

	switch (request.params.key) {
		case "name":
			results = await Template.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await Template.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
router.post("/one", async (request, response) => {
	// let newAbility = await Template.create({
	...
	let newDoc = await Template.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await Template.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await Template.findOneAndDelete({_id: request.body.targetDocId});
