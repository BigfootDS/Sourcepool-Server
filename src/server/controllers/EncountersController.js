...
router.get("/all", async (request, response) => {
	let result = await Encounter.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await Encounter.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await Encounter.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await Encounter.find({});

	switch (request.params.key) {
		case "name":
			results = await Encounter.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await Encounter.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
/*
{
    ...
    "description": [
        {
            "language":"en",
            "name":"Testo Encounter",
            "content":"Test of functionality."
        },
                {
            "language":"fr",
            "name":"Encounter de Testo",
...
router.post("/one", async (request, response) => {
	// let newAbility = await Encounter.create({
	...
	let newDoc = await Encounter.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await Encounter.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await Encounter.findOneAndDelete({_id: request.body.targetDocId});
