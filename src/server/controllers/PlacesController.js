...
router.get("/all", async (request, response) => {
	let result = await Place.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await Place.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await Place.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await Place.find({});

	switch (request.params.key) {
		case "name":
			results = await Place.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await Place.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
router.post("/one", async (request, response) => {
	// let newAbility = await Place.create({
	...
	let newDoc = await Place.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await Place.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await Place.findOneAndDelete({_id: request.body.targetDocId});
