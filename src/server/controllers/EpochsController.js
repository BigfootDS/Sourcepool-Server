...
router.get("/all", async (request, response) => {
	let result = await Epoch.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await Epoch.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await Epoch.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await Epoch.find({});

	switch (request.params.key) {
		case "name":
			results = await Epoch.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await Epoch.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
router.post("/one", async (request, response) => {
	// let newAbility = await Epoch.create({
	...
	let newDoc = await Epoch.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await Epoch.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await Epoch.findOneAndDelete({_id: request.body.targetDocId});
