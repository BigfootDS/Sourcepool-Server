...
router.get("/all", async (request, response) => {
	let result = await InGameEvent.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await InGameEvent.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await InGameEvent.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await InGameEvent.find({});

	switch (request.params.key) {
		case "name":
			results = await InGameEvent.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await InGameEvent.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
/*
{
    ...
    "description": [
        {
            "language":"en",
            "name":"Testo InGameEvent",
            "content":"Test of functionality."
        },
                {
            "language":"fr",
            "name":"InGameEvent de Testo",
...
router.post("/one", async (request, response) => {
	// let newAbility = await InGameEvent.create({
	...
	let newDoc = await InGameEvent.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await InGameEvent.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await InGameEvent.findOneAndDelete({_id: request.body.targetDocId});
