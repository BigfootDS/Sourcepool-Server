...
router.get("/all", async (request, response) => {
	let result = await Campaign.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await Campaign.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await Campaign.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await Campaign.find({});

	switch (request.params.key) {
		case "name":
			results = await Campaign.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await Campaign.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
router.post("/one", async (request, response) => {
	// let newAbility = await Campaign.create({
	...
	let newDoc = await Campaign.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await Campaign.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await Campaign.findOneAndDelete({_id: request.body.targetDocId});
