...
router.get("/all", async (request, response) => {
	let result = await Character.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await Character.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await Character.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await Character.find({});

	switch (request.params.key) {
		case "name":
			results = await Character.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await Character.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
/*
{
    ...
    "description": [
        {
            "language":"en",
            "name":"Testo Character",
            "content":"Test of functionality."
        },
                {
            "language":"fr",
            "name":"Character de Testo",
...
router.post("/one", async (request, response) => {
	// let newAbility = await Character.create({
	...
	let newDoc = await Character.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await Character.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await Character.findOneAndDelete({_id: request.body.targetDocId});
