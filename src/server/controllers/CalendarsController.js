...
router.get("/all", async (request, response) => {
	let result = await Calendar.find({}).catch(error => error);
...
router.get("/one/:id", async (request, response) => {
	let result = await Calendar.findOne({_id: request.params.id}).catch(error => error);
...
router.get("/multiple/:key/:value", async (request, response) => {
	...
	//let result = await Calendar.find({[request.params.key]: request.params.value}).catch(error => error);
	let results = [];

	// let allDocuments = await Calendar.find({});

	switch (request.params.key) {
		case "name":
			results = await Calendar.find({"description.name": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		case "tags":
		case "tag":
			results = await Calendar.find({"tags": {$regex: new RegExp(`(${request.params.value})`, 'gi')}}).catch(error => error);
			break;
		default:
			break;
	}
...
router.post("/one", async (request, response) => {
	// let newAbility = await Calendar.create({
	...
	let newDoc = await Calendar.create(request.body).save().catch(error => error);
...
router.patch("/one", async (request, response) => {
	let result = await Calendar.findOne({_id: request.body.targetDocId});
...
router.delete("/one", async (request, response) => {
	let result = await Calendar.findOneAndDelete({_id: request.body.targetDocId});
